...
function pcRender(){
  ...
  let bodyHtml;
  if(ms.length){
    bodyHtml=`<div class="pc-thread">${ms.map(m=>m.role==='user'?`<div class="pc-msg you"><div class="pc-bub">${esc(m.text)}${attRowHtml(m.atts)}</div></div>`:`<div class="pc-msg"><div class="pc-bub">${mdToHtml(m.text)}</div></div>`).join('')}${pcBusy?'<div class="pc-msg"><div class="pc-bub dim">lendo o projeto…</div></div>':''}</div>`;
  } else {
