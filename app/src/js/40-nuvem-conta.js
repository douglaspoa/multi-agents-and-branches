...
async function renderCloud(){
  ...
  const teamsHtml = d.teams.map(t=>{
    ...
    const leadTag=leadNames.length?`<span class="dim" style="font-size:10.5px">lead: ${esc(leadNames.join(', '))}</span>`:'<span class="dim" style="font-size:10.5px;color:var(--warn)">sem lead</span>';
    return `<div class="tmcard" style="margin-bottom:10px"><div class="tmrow1"><b class="tmtitle">${esc(t.name)}</b>${canManage(t)?`<button class="btn sm" data-mact="rename" data-team="${escA(t.id)}" title="renomear time" style="padding:1px 7px;font-size:10px">✎</button>`:''}<span class="dim" style="font-size:11px">${mems.length} membro${mems.length===1?'':'s'}</span><span style="flex:1"></span>${t.id===teamId?'<span class="tmbadge" style="color:var(--good);border-color:var(--good)">time atual</span>':`<button class="btn sm" data-mact="use" data-team="${escA(t.id)}">usar este time</button>`}${isAdmin?`<button class="btn sm" data-mact="delteam" data-team="${escA(t.id)}" title="excluir time" style="padding:1px 7px;font-size:10px;color:var(--crit)">✕</button>`:''}</div><div style="margin:2px 0 6px">${leadTag} · <span class="dim" style="font-size:10.5px">só o time vê o que ele faz</span></div><div class="costlist" style="margin-top:4px">${rows}</div>${addRow}</div>`;
  }).join('');
  ...
  body.innerHTML = cloudMsgHtml()+`
    ...
    <div class="costlist">${(d.orgMembers||[]).map(om=>`<div class="costrow"><span class="cav" style="background:${agentColor(pName(om.user_id))}">${esc(pName(om.user_id).slice(0,2).toUpperCase())}</span><span class="cnm">${esc(pName(om.user_id))}${om.user_id===cloudUserId()?' <span class="dim">(você)</span>':''}</span><span class="dim" style="font-size:11px">${esc(om.role)}</span>${(om.role!=='owner'&&om.user_id!==cloudUserId())?`<button class="btn sm" data-orgrm="${escA(om.user_id)}" title="remove da organização e de todos os times — libera o assento">remover da org</button>`:''}</div>`).join('')}</div>`:''}
