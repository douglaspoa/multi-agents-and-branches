...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const perSeat = !!plan.per_seat;
    const seats = plan.plan === "team" && perSeat ? Math.min(cap, Math.max(1, parseInt(String(seatsIn ?? 1), 10) || 1)) : 1;
